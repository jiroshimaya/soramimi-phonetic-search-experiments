"""非推論条件のプロンプトバリエーション追加試行（v2〜v5）を OpenAI Batch API で実行するスクリプト

使い方（リポジトリルートで）:
  uv run reproduce_leaderboard/methods/012_prompt_variations_nonreasoning_batch.py preview
  uv run reproduce_leaderboard/methods/012_prompt_variations_nonreasoning_batch.py submit
  uv run reproduce_leaderboard/methods/012_prompt_variations_nonreasoning_batch.py status
  uv run reproduce_leaderboard/methods/012_prompt_variations_nonreasoning_batch.py retrieve

条件は 008_01〜008_06 の full 評価（非推論5条件）に対応し、各条件について
prompt_variations.py の v2〜v5 を1試行ずつ実行する（v1 は既存の 008_* 結果）。
"""
...
# 008_* の full 評価5条件に対応
CONDITIONS = {
    "simple": {
        "prompt_key": "simple",
        "input_transform": "none",
        "rank_func": "llm_rerank",
        "baseline_script": "008_01",
    },
    "detailed": {
        "prompt_key": "detailed",
        "input_transform": "none",
        "rank_func": "llm_rerank",
        "baseline_script": "008_02",
    },
    "step_by_step": {
        "prompt_key": "step_by_step",
        "input_transform": "none",
        "rank_func": "llm_rerank",
        "baseline_script": "008_03",
    },
    "detailed_romaji": {
        "prompt_key": "detailed",
        "input_transform": "pyopenjtalk_romaji",
        "rank_func": "vowel_consonant",
        "baseline_script": "008_04",
    },
    "detailed_kana_romaji": {
        "prompt_key": "detailed",
        "input_transform": "kana_and_pyopenjtalk_romaji",
        "rank_func": "vowel_consonant",
        "baseline_script": "008_06",
    },
}

OUTPUT_DIR = METHODS_DIR.parent / "results" / "012_prompt_variations"
...
def state_path_for(condition: str, variant: str) -> Path:
    return OUTPUT_DIR / f"012_{condition}_{variant}_openai_batch_state.json"


def result_path_for(condition: str, variant: str) -> Path:
    return OUTPUT_DIR / f"012_{condition}_{variant}.json"
