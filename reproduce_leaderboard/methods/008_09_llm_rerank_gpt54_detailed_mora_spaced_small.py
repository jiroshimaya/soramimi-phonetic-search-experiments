"""LLMリランク (gpt-5.4, detailed prompt, mora-spaced input, small dataset_for_llm) による評価を実行するスクリプト"""
...
INPUT_TRANSFORM = "mora_spaced"
PROMPT_INSTRUCTIONS = """
クエリ（Query）と単語一覧（Wordlist）が与えられます。
クエリと発音が似ている順に、単語一覧を並び替えてください。
- Query と Wordlist のカタカナはモーラごとにスペース区切りで与えられます
- 子音より母音の一致を優先してください
- クエリとモーラ数が同じであることを優先してください。ただし促音（ッ）、撥音（ン）、長音（「ー」や直前のカナの母音と同じ単母音モーラ、エ段のカナの直後のイ、オ段のカナの直後のウなど）の挿入や削除は許容されます
- 文字単位ではなく、モーラ単位での対応を重視してください
...
def mora_spaced(text: str) -> str:
    return " ".join(jamorasep.parse(text))
...
def build_ranking_function():
    def ranking_func(
        ...
        spaced_queries = [mora_spaced(query) for query in query_texts]
        spaced_wordlists = [
            [mora_spaced(word) for word in wordlist] for wordlist in wordlists
        ]
...
def main():
    ...
    output_path = output_dir / "008_09_llm_rerank_gpt54_detailed_mora_spaced.json"
    ...
    results.parameters.metadata.update(
        {
            ...
            "dataset_size": "small",
            "dataset_loader": "default_dataset_for_llm(query_limit=SMALL_DATASET_QUERY_COUNT)",
        }
    )
